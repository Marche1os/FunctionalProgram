...
data class Match(
    ...
    val width: Int,
    val height: Int,
    private val rawPattern: Collection<Position>
) {
    // Создаем защитную копию списка (read-only), как в оригинале
    val pattern: List<Position> = rawPattern.toList()

    init {
        // Проверяем, что все относительные позиции находятся внутри прямоугольника
        for (pos in pattern) {
            val isValid = pos.row in 0 until height && pos.col in 0 until width
            require(isValid) { "Relative position $pos outside bounds [Height: $height, Width: $width]" }
        }
    }

    fun getAbsolutePositions(): List<Position> {
        // pattern.map создает новый список с преобразованными элементами
        return pattern.map { rp ->
            Position(origin.row + rp.row, origin.col + rp.col)
        }
    }
...
//            if (length >= 3) {
//                matches.add(Match(direction.name, row, col, length))
//            }
//        }
